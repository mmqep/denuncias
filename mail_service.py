...
def enviar_correo(destinatario_email, destinatario_nombre, asunto, cuerpo_html, timeout=10):
    ...
    if not all([SMTP_SERVER, SMTP_USERNAME, SMTP_PASSWORD, SMTP_FROM_EMAIL]):
        logger.warning(
            "Configuración SMTP incompleta. No se envió notificación a %s.",
            destinatario_email,
        )
        return False
    ...
    try:
        ...
        logger.error(
            "Error de autenticación SMTP al notificar a %s.", destinatario_email
        )
    except smtplib.SMTPConnectError as exc:
        logger.error(
            "No se pudo conectar al servidor SMTP (%s:%s): %s",
            SMTP_SERVER, SMTP_PORT, exc,
        )
    except TimeoutError:
        logger.error(
            "Timeout al conectar con el servidor SMTP para notificar a %s.",
            destinatario_email,
        )
    except smtplib.SMTPException as exc:
        logger.error(
            "Error SMTP al enviar notificación a %s: %s", destinatario_email, exc
        )
